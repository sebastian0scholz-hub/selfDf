...
// Kontrollklasse für Bildschirme (Inklusive Mobil-Layout & Touch-Buttons)
class ResponsiveScreen {
    ...
    setupMobileControls() {
        ...
        // 2. Rosenberg-Skala Buttons (1-4) auf dem Smartphone
            ...
            document.getElementById('btn-1').onclick = () => this.handleKey('1');
            document.getElementById('btn-2').onclick = () => this.handleKey('2');
    ...
    draw() {
        ...
        let scaleFactor = isMobile ? 2.0 : 1.0;
    ...
    handleKey(key) {
        ...
        this.active = false;
        canvas.onclick = null;
        document.getElementById('mobile-binary-buttons').style.display = 'none';
        document.getElementById('mobile-number-buttons').style.display = 'none';
...
function startExperiment() {
    routines.push(() => {
        currentRoutine = new ResponsiveScreen("Willkommen zu diesem Experiment.\n\nSchön, dass Sie teilnehmen!", isMobile ? "[ZUM FORTFAHREN AUF DEN BILDSCHIRM TIPPEN]" : "[LEERTASTE DRÜCKEN ZUM FORTFAHREN]", [' '], nextRoutine);
        currentRoutine.draw();
    });
    routines.push(() => {
        currentRoutine = new ResponsiveScreen("Es folgen einige Fragen zu Ihrer Person.\nBitte nutzen Sie die Tasten zum Antworten.", isMobile ? "[ZUM FORTFAHREN AUF DEN BILDSCHIRM TIPPEN]" : "[LEERTASTE DRÜCKEN]", [' '], nextRoutine);
        currentRoutine.draw();
    });
    ...
    rosenbergItems.forEach(item => {
        routines.push(() => {
            currentRoutine = new ResponsiveScreen(item, isMobile ? "" : "1 = Trifft gar nicht zu\n2 = Trifft eher nicht zu\n3 = Trifft eher zu\n4 = Trifft voll zu", ['1','2','3','4'], (key, rt) => {
    ...
    routines.push(() => {
        ...
        currentRoutine = new ResponsiveScreen(text, isMobile ? "[ZUM STARTEN AUF DEN BILDSCHIRM TIPPEN]" : "[LEERTASTE ZUM STARTEN]", [' '], nextRoutine);
        currentRoutine.draw();
    });
    learningItems.forEach(item => {
        routines.push(() => {
            let baseSize = Math.max(canvas.width, canvas.height);
            let fixSize = isMobile ? Math.round(baseSize*0.0275*2) : Math.round(baseSize*0.0275);
            currentRoutine = { draw: () => { ctx.fillStyle = "#7F7F7F"; ctx.fillRect(0,0,canvas.width,canvas.height); ctx.fillStyle = "#000000"; ctx.font = "bold " + fixSize + "px Arial"; ctx.textAlign = "center"; ctx.fillText("+", canvas.width/2, canvas.height/2); }, handleKey: () => {} };
            currentRoutine.draw();
            setTimeout(() => {
                currentRoutine = new ResponsiveScreen(item.word.toUpperCase(), item.prompt, ['f', 'j'], (key, rt) => {
                    ...
                    currentRoutine = { draw: () => {
                        ...
                        let cueSize = isMobile ? Math.round(Math.max(canvas.width, canvas.height)*0.025*2) : Math.round(Math.max(canvas.width, canvas.height)*0.025);
    ...
    routines.push(() => {
        ...
        currentRoutine = new ResponsiveScreen(text, isMobile ? "[ZUM STARTEN AUF DEN BILDSCHIRM TIPPEN]" : "[LEERTASTE DRÜCKEN ZUM STARTEN]", [' '], runDistractorGame);
        currentRoutine.draw();
    });
    routines.push(() => {
    ...
    routines.push(() => {
        currentRoutine = new ResponsiveScreen("Zum Abschluss bitten wir Sie noch um die Beantwortung von fünf kurzen Fragen.", isMobile ? "[ZUM FORTFAHREN TIPPEN]" : "[LEERTASTE DRÜCKEN]", [' '], nextRoutine);
        currentRoutine.draw();
    });
    ...
    routines.push(() => {
        ...
        currentRoutine = { draw: () => {
            ...
            let baseSize = Math.max(canvas.width, canvas.height);
            let endSize = isMobile ? Math.round(baseSize * 0.011 * 2) : Math.round(baseSize * 0.011);
            ctx.font = "bold " + endSize + "px Arial";
